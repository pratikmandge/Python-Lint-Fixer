...
export class PythonFormatter {
    ...
    private isInDocstring(lineIndex: number, lines: string[]): boolean {
    ...
    private formatImports(text: string): string {
        const lines = text.split('\n');
        const importLines: { line: number; content: string; type: 'import' | 'from'; isMultiLine: boolean }[] = [];
        const nonImportLines: { line: number; content: string }[] = [];
    ...
    private groupAndSortImports(importLines: { line: number; content: string; type: 'import' | 'from'; isMultiLine: boolean }[]): string[][] {
        // Remove duplicates first
        const uniqueImports = importLines.filter((importLine, index, self) => 
            index === self.findIndex(t => t.content === importLine.content)
        );
    ...
    private isSystemModule(moduleName: string): boolean {
        const systemModules = [
    ...
    private groupProjectImports(projectImports: string[]): string[][] {
    ...
    private fixLineLength(text: string): string {
        const lines = text.split('\n');
        const fixedLines: string[] = [];
    ...
    private wrapImportLine(line: string): string[] {
    ...
    private wrapFunctionDefinition(lines: string[], startIndex: number): { lines: string[], nextIndex: number } {
        const result: string[] = [];
    ...
    private ensureFileEnding(text: string): string {
