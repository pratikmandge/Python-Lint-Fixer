...
class PythonLinter {
    ...
    lintLine(line, lineNumber, document) {
        const issues = [];
        const range = new vscode.Range(lineNumber - 1, 0, lineNumber - 1, line.length);
        // Check line length
        if (line.length > this.maxLineLength) {
            issues.push(new vscode.Diagnostic(range, `Line exceeds ${this.maxLineLength} characters (${line.length})`, vscode.DiagnosticSeverity.Warning));
        }
    ...
    checkQuotes(line, lineNumber, document) {
        const issues = [];
        const text = document.getText();
        const lines = text.split('\n');
        // Skip docstrings
        ...
        while ((match = doubleQuoteRegex.exec(line)) !== null) {
            ...
            issues.push(new vscode.Diagnostic(range, 'Use single quotes instead of double quotes', vscode.DiagnosticSeverity.Warning));
        }
        return issues;
    }
    isInDocstring(lineNumber, lines) {
    ...
    requiresDoubleQuotes(content) {
        // Check if content requires double quotes (contains single quotes)
        return content.includes("'");
    }
    checkImportFormat(line, lineNumber, document) {
        const issues = [];
        const range = new vscode.Range(lineNumber - 1, 0, lineNumber - 1, line.length);
        ...
        if (line.trim().startsWith('import ')) {
            ...
            if (importMatch) {
                ...
                if (modules.length > 1) {
                    issues.push(new vscode.Diagnostic(range, 'Multiple imports should be on separate lines', vscode.DiagnosticSeverity.Warning));
                }
            }
        }
        return issues;
    }
    checkSpacing(line, lineNumber, document) {
        const issues = [];
        const text = document.getText();
        const lines = text.split('\n');
        // Check class spacing
    ...
    checkClassSpacing(lineNumber, lines) {
        const issues = [];
        ...
        if (lineNumber > 2) {
            ...
            if (prevLine1.trim() !== '' || prevLine2.trim() !== '') {
                ...
                issues.push(new vscode.Diagnostic(range, 'Classes should be preceded by 2 blank lines', vscode.DiagnosticSeverity.Warning));
            }
        }
        return issues;
    }
    checkMethodSpacing(lineNumber, lines) {
        const issues = [];
        ...
        if (lineNumber > 1) {
            ...
            if (!isFirstMethodInClass && prevLine.trim() !== '') {
                const range = new vscode.Range(lineNumber - 1, 0, lineNumber - 1, 0);
                issues.push(new vscode.Diagnostic(range, 'Methods should be preceded by 1 blank line', vscode.DiagnosticSeverity.Warning));
            }
        }
        return issues;
    }
    lintFile(text, document) {
        const issues = [];
        const lines = text.split('\n');
        // Check if file ends with blank line
        if (lines.length > 0 && lines[lines.length - 1].trim() !== '') {
            const lastLineRange = new vscode.Range(lines.length - 1, 0, lines.length - 1, 0);
            issues.push(new vscode.Diagnostic(lastLineRange, 'File should end with a blank line', vscode.DiagnosticSeverity.Warning));
        }
    ...
    checkImportGrouping(text, document) {
        const issues = [];
        const lines = text.split('\n');
        const importLines = [];
        ...
        if (importLines.length > 1) {
            for (let i = 1; i < importLines.length; i++) {
                ...
                if (linesBetween < 1) {
                    ...
                    issues.push(new vscode.Diagnostic(range, 'Import groups should be separated by at least 1 blank line', vscode.DiagnosticSeverity.Warning));
                }
            }
        }
        return issues;
